...
const ArticleForm = ({ edit }) => {
    ...
    const onSubmit = async (data) => {
        ...
        const res = edit ? await blogData.updateArticle(token, articleData, location.state.slug) : await blogData.createArticle(token, articleData)
            ...
            <form className={classes.form__main} onSubmit={handleSubmit(onSubmit)}>
                <fieldset className={classes.form__content}>
                    ...
                    <label className={classes.form__item}>
                        Title
                        <input defaultValue={edit ? location.state.title : ''} placeholder="Title" className={classes.form__input} {...register('title', {
                    ...
                    <label className={classes.form__item}>
                        Short description
                        <input defaultValue={edit ? location.state.description : ''} placeholder="Description" className={classes.form__input} {...register('description', {
                    ...
                    <label className={classes.form__item}>
                        Text
                        <textarea defaultValue={edit ? location.state.body : ''} placeholder="Text" className={classes.form__text} {...register('body', {
                    ...
                    <label className={`${classes.form__item} ${classes.form__tags}`}>
                        ...
                        {fields.map((field, index) => (
                            <div key={field.id} className={classes.form__tag}>
                                <input placeholder="Tag" className={classes.form__taginput} {...register(`tags[${index}].value`, {
                                    required: 'The tag must not be empty',
                                    minLength: 1,
                                    maxLength: 20,
                                })}></input>
                                <button type="button" className={classes.form__delete} onClick={() => remove(index)}>Delete</button>
                                <button type="button" className={classes.form__add} onClick={() => append({ name: '' })}>Add tag</button>
                    ...
                    {fields.length === 0 && (
                        <button type="button" className={classes.form__tagButton} onClick={() => append({ name: ''})}>Add tag</button>
                    )}
                     
                    <Button className={classes.form__button} loading={loading} htmlType='submit' type='primary'>Send</Button>
                </fieldset>
            </form>
        </div>
    )
