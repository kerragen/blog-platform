...
const Article = ({ title, author, tagList, body, created, description, favorited, favoritesCount, slug, updatedAt, full}) => {
    ...
    const tags = [...new Set(tagList)].slice(0, 9).map((tag) => {
        ...
        return (
            <li key={tag} className={full ? classes.article__tagFull : classes.article__tag}>{tag}</li>
        )
    })
    ...
    const DeleteButton = () => {
        return (
            <Popconfirm title="Are you sure to delete this article?" okText="Yes" cancelText="No" onConfirm={deleteArticle}>
                <Button className={classes.article__delete} danger>Delete</Button>
            </Popconfirm>
        )
    }
    return (
        <div className={full ? classes.articleFull : classes.article}>
           <div className={classes.article__header}>
                <div>
                    {full ? (<span className={classes.article__title}>{title}</span>) : (<Link className={classes.article__title} to={`/articles/${slug}`}>{title}</Link>)}
